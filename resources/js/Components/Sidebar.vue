...
// Более точная проверка через Ziggy
...
<template>
    <aside
        class="fixed inset-y-0 left-0 z-50 w-72 bg-gray-100 shadow-lg flex-shrink-0 hidden md:block h-screen overflow-y-auto">
        <div class="flex flex-col h-full p-8">
            <!-- Логотип -->
            <img src="/images/daily-grow-logo.png" alt="Daily Grow"
                class="h-7 w-auto mx-auto md:mx-0 object-contain mb-3" />
            ...
            <!-- Блок-заголовок -->
            <div
                class="flex items-center px-6 py-2 mb-4 rounded-2xl font-medium text-gray-700 bg-white shadow-sm min-h-[48px]">
                <svg class="w-5 h-5 mr-4 flex-shrink-0 opacity-20" viewBox="0 0 24 24" fill="none"
            ...
            <nav class="space-y-1">
                <Link :href="route('reviews.index')" :class="[
                    'flex items-center px-4 py-2 rounded-xl font-medium transition-colors',
                    isActive('reviews.index') ? 'bg-white shadow-sm' : 'text-gray-700 hover:bg-white/50'
                ]">
                    Отзывы
                </Link>

                <Link :href="route('settings.edit')" :class="[
                    'flex items-center px-4 py-2 rounded-xl font-medium transition-colors',
                    isActive('settings.edit') ? 'bg-white shadow-sm' : 'text-gray-700 hover:bg-white/50'
                ]">
                    Настройки
                </Link>
            </nav>
